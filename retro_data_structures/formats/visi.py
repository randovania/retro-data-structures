...
OctreeNode = Struct(
    "header" / BitsSwapped(BitStruct(
        "subdivide_x" / Flag,
        "subdivide_y" / Flag,
        "subdivide_z" / Flag,
        "node_type" / Enum(
            BitsInteger(2, swapped=True),
            out_of_bounds=1,
            end_of_hierarchy=2,
            regular_node=3
        ),
...
VISI = Struct(
    ...
    "light_visibility_nodes" / Array(
        this.light_visibility_node_count,
        FixedSized(this.leaf_size, GreedyBytes)),
    "bounding_box" / AABox,
