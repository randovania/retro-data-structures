...
class Water(BaseObjectType):
    ...
    trigger_properties: TriggerInfo = dataclasses.field(
        default_factory=TriggerInfo,
        metadata={
            "reflection": FieldReflection[TriggerInfo](
                TriggerInfo,
                id=0xBBFEE93E,
                original_name="TriggerProperties",
                from_json=TriggerInfo.from_json,
                to_json=TriggerInfo.to_json,
            ),
        },
    )
    fluid_properties: FluidProperties = dataclasses.field(
        default_factory=FluidProperties,
        metadata={
            "reflection": FieldReflection[FluidProperties](
                FluidProperties,
                id=0x274D1E54,
                original_name="FluidProperties",
                from_json=FluidProperties.from_json,
                to_json=FluidProperties.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        trigger_properties = TriggerInfo.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x274D1E54
        fluid_properties = FluidProperties.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.trigger_properties.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.fluid_properties.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_trigger_properties(data: typing.BinaryIO, game: Game, property_size: int) -> TriggerInfo:
    return TriggerInfo.from_stream(data, game, property_size)


def _decode_fluid_properties(data: typing.BinaryIO, game: Game, property_size: int) -> FluidProperties:
    return FluidProperties.from_stream(data, game, property_size)
