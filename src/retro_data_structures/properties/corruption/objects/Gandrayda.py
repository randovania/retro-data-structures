...
class Gandrayda(BaseObjectType):
    ...
    gandrayda_data_0xfee49b25: GandraydaData = dataclasses.field(
        default_factory=GandraydaData,
        metadata={
            "reflection": FieldReflection[GandraydaData](
                GandraydaData,
                id=0xFEE49B25,
                original_name="GandraydaData",
                from_json=GandraydaData.from_json,
                to_json=GandraydaData.to_json,
            ),
        },
    )
    gandrayda_data_0x49b975e6: GandraydaData = dataclasses.field(
        default_factory=GandraydaData,
        metadata={
            "reflection": FieldReflection[GandraydaData](
                GandraydaData,
    ...
    gandrayda_data_0x82439cba: GandraydaData = dataclasses.field(
        default_factory=GandraydaData,
        metadata={
            "reflection": FieldReflection[GandraydaData](
                GandraydaData,
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        patterned = PatternedAITypedef.from_stream(
            data, game, property_size, default_override={"step_up_height": 1.0, "creature_size": 1, "turn_speed": 60.0}
        )
        ...
        gandrayda_data_0xfee49b25 = GandraydaData.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x49B975E6
        gandrayda_data_0x49b975e6 = GandraydaData.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x82439CBA
        gandrayda_data_0x82439cba = GandraydaData.from_stream(data, game, property_size)

        return cls(
            editor_properties,
            patterned,
            actor_information,
            gandrayda_data_0xfee49b25,
            gandrayda_data_0x49b975e6,
            gandrayda_data_0x82439cba,
        )
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        self.patterned.to_stream(
            data, game, default_override={"step_up_height": 1.0, "creature_size": 1, "turn_speed": 60.0}
        )
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.gandrayda_data_0xfee49b25.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.gandrayda_data_0x49b975e6.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.gandrayda_data_0x82439cba.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_gandrayda_data_0xfee49b25(data: typing.BinaryIO, game: Game, property_size: int) -> GandraydaData:
    return GandraydaData.from_stream(data, game, property_size)


def _decode_gandrayda_data_0x49b975e6(data: typing.BinaryIO, game: Game, property_size: int) -> GandraydaData:
    return GandraydaData.from_stream(data, game, property_size)


def _decode_gandrayda_data_0x82439cba(data: typing.BinaryIO, game: Game, property_size: int) -> GandraydaData:
    return GandraydaData.from_stream(data, game, property_size)
