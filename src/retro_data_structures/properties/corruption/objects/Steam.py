...
class Steam(BaseObjectType):
    ...
    steam: AssetId = dataclasses.field(
        default=default_asset_id,
        metadata={
            "asset_types": ["TXTR"],
            "reflection": FieldReflection[AssetId](AssetId, id=0x26346050, original_name="Steam"),
        },
    )
    strength: float = dataclasses.field(
        default=0.3499999940395355,
        metadata={
            "reflection": FieldReflection[float](float, id=0x4F8F5F5C, original_name="Strength"),
        },
    )
    fade_in_rate: float = dataclasses.field(
        default=1.0,
        metadata={
            "reflection": FieldReflection[float](float, id=0xC2138F3D, original_name="FadeInRate"),
        },
    )
    fade_out_rate: float = dataclasses.field(
        default=2.0,
        metadata={
            "reflection": FieldReflection[float](float, id=0x2E9F259E, original_name="FadeOutRate"),
        },
    )
    ...
    unknown_0xa366c949: bool = dataclasses.field(
        default=False,
        metadata={
            "reflection": FieldReflection[bool](bool, id=0xA366C949, original_name="Unknown"),
        },
    )
    unknown_0x606539fa: bool = dataclasses.field(
        default=False,
        metadata={
            "reflection": FieldReflection[bool](bool, id=0x606539FA, original_name="Unknown"),
        },
    )
    filter_shape: FilterShape = dataclasses.field(
        default=FilterShape.Unknown1,
        metadata={
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        steam = structs.BIG_Q.unpack(data.read(8))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x4F8F5F5C
        strength = structs.BIG_f.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xC2138F3D
        fade_in_rate = structs.BIG_f.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x2E9F259E
        fade_out_rate = structs.BIG_f.unpack(data.read(4))[0]
        ...
        unknown_0xa366c949 = structs.BIG_bool_.unpack(data.read(1))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x606539FA
        unknown_0x606539fa = structs.BIG_bool_.unpack(data.read(1))[0]
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(structs.BIG_Q.pack(self.steam))

        data.write(b"O\x8f_\\")  # 0x4f8f5f5c
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_f.pack(self.strength))

        data.write(b"\xc2\x13\x8f=")  # 0xc2138f3d
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_f.pack(self.fade_in_rate))

        data.write(b".\x9f%\x9e")  # 0x2e9f259e
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_f.pack(self.fade_out_rate))
        ...
        data.write(structs.BIG_bool_.pack(self.unknown_0xa366c949))

        data.write(b"`e9\xfa")  # 0x606539fa
        data.write(b"\x00\x01")  # size
        data.write(structs.BIG_bool_.pack(self.unknown_0x606539fa))
...
def _decode_filter_shape(data: typing.BinaryIO, game: Game, property_size: int) -> FilterShape:
    return FilterShape.from_stream(data, game)


_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    0x255A4580: ("editor_properties", _decode_editor_properties),
    0x77A27411: ("trigger", _decode_trigger),
    0x26346050: ("steam", structs.decode_BIG_Q),
    0x4F8F5F5C: ("strength", structs.decode_BIG_f),
    0xC2138F3D: ("fade_in_rate", structs.decode_BIG_f),
    0x2E9F259E: ("fade_out_rate", structs.decode_BIG_f),
    0x78C507EB: ("radius", structs.decode_BIG_f),
    0xA366C949: ("unknown_0xa366c949", structs.decode_BIG_bool_),
    0x606539FA: ("unknown_0x606539fa", structs.decode_BIG_bool_),
