...
class Ship(BaseObjectType):
    ...
    ship_data: ShipData = dataclasses.field(
        default_factory=ShipData,
        metadata={
            "reflection": FieldReflection[ShipData](
                ShipData,
                id=0xC854D4A6,
                original_name="ShipData",
                from_json=ShipData.from_json,
                to_json=ShipData.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        ship_data = ShipData.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.ship_data.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_ship_data(data: typing.BinaryIO, game: Game, property_size: int) -> ShipData:
    return ShipData.from_stream(data, game, property_size)
