...
class ShellBug(BaseObjectType):
    ...
    shell_bug_data: ShellBugData = dataclasses.field(
        default_factory=ShellBugData,
        metadata={
            "reflection": FieldReflection[ShellBugData](
                ShellBugData,
                id=0xB9FFA928,
                original_name="ShellBugData",
                from_json=ShellBugData.from_json,
                to_json=ShellBugData.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        shell_bug_data = ShellBugData.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.shell_bug_data.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_shell_bug_data(data: typing.BinaryIO, game: Game, property_size: int) -> ShellBugData:
    return ShellBugData.from_stream(data, game, property_size)
