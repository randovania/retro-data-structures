...
class Korakk(BaseObjectType):
    ...
    unknown_struct44: UnknownStruct44 = dataclasses.field(
        default_factory=UnknownStruct44,
        metadata={
            "reflection": FieldReflection[UnknownStruct44](
                UnknownStruct44,
                id=0x85BB6891,
                original_name="UnknownStruct44",
                from_json=UnknownStruct44.from_json,
                to_json=UnknownStruct44.to_json,
            ),
        },
    )
    korakk_data_0xadb462e2: KorakkData = dataclasses.field(
        default_factory=KorakkData,
        metadata={
            "reflection": FieldReflection[KorakkData](
                KorakkData,
                id=0xADB462E2,
                original_name="KorakkData",
                from_json=KorakkData.from_json,
                to_json=KorakkData.to_json,
            ),
        },
    )
    ...
    korakk_data_0xc8e90b50: KorakkData = dataclasses.field(
        default_factory=KorakkData,
        metadata={
            "reflection": FieldReflection[KorakkData](
                KorakkData,
    ...
    korakk_data_0xba37072a: KorakkData = dataclasses.field(
        default_factory=KorakkData,
        metadata={
            "reflection": FieldReflection[KorakkData](
                KorakkData,
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        unknown_struct44 = UnknownStruct44.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xADB462E2
        korakk_data_0xadb462e2 = KorakkData.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xB3774750
        patterned = PatternedAITypedef.from_stream(
            data, game, property_size, default_override={"turn_speed": 65.0, "detection_range": 5.0}
        )

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xC8E90B50
        korakk_data_0xc8e90b50 = KorakkData.from_stream(data, game, property_size)
        ...
        korakk_data_0xba37072a = KorakkData.from_stream(data, game, property_size)
        ...
        return cls(
            editor_properties,
            unknown_struct44,
            korakk_data_0xadb462e2,
            patterned,
            korakk_data_0xc8e90b50,
            patterned_ai_0x1464ae05,
            korakk_data_0xba37072a,
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.unknown_struct44.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.korakk_data_0xadb462e2.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.patterned.to_stream(data, game, default_override={"turn_speed": 65.0, "detection_range": 5.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.korakk_data_0xc8e90b50.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.patterned_ai_0x1464ae05.to_stream(
            data, game, default_override={"turn_speed": 65.0, "detection_range": 5.0}
        )
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.korakk_data_0xba37072a.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.patterned_ai_0x24d00673.to_stream(
            data, game, default_override={"turn_speed": 65.0, "detection_range": 5.0}
        )
        after = data.tell()
        data.seek(before)
...
def _decode_unknown_struct44(data: typing.BinaryIO, game: Game, property_size: int) -> UnknownStruct44:
    return UnknownStruct44.from_stream(data, game, property_size)


def _decode_korakk_data_0xadb462e2(data: typing.BinaryIO, game: Game, property_size: int) -> KorakkData:
    return KorakkData.from_stream(data, game, property_size)
...
def _decode_korakk_data_0xc8e90b50(data: typing.BinaryIO, game: Game, property_size: int) -> KorakkData:
    return KorakkData.from_stream(data, game, property_size)
...
def _decode_korakk_data_0xba37072a(data: typing.BinaryIO, game: Game, property_size: int) -> KorakkData:
    return KorakkData.from_stream(data, game, property_size)
