...
if typing.TYPE_CHECKING:
    ...
    class FargullHatcherSwarmJson(typing_extensions.TypedDict):
        ...
        fargull_hatcher_swarm_data: json_util.JsonObject
...
class FargullHatcherSwarm(BaseObjectType):
    ...
    flyer_swarm_data: FlyerSwarmData = dataclasses.field(
        default_factory=FlyerSwarmData,
        metadata={
            "reflection": FieldReflection[FlyerSwarmData](
                FlyerSwarmData,
                id=0x99C18702,
                original_name="FlyerSwarmData",
                from_json=FlyerSwarmData.from_json,
                to_json=FlyerSwarmData.to_json,
            ),
        },
    )
    fargull_hatcher_swarm_data: FargullHatcherSwarmData = dataclasses.field(
        default_factory=FargullHatcherSwarmData,
        metadata={
            "reflection": FieldReflection[FargullHatcherSwarmData](
                FargullHatcherSwarmData,
                id=0x146F75D9,
                original_name="FargullHatcherSwarmData",
                from_json=FargullHatcherSwarmData.from_json,
                to_json=FargullHatcherSwarmData.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        flyer_swarm_data = FlyerSwarmData.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x146F75D9
        fargull_hatcher_swarm_data = FargullHatcherSwarmData.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.flyer_swarm_data.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.fargull_hatcher_swarm_data.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
    ...
    def from_json(cls, data: json_util.JsonValue) -> typing_extensions.Self:
        json_data = typing.cast("FargullHatcherSwarmJson", data)
        return cls(
            ...
            fargull_hatcher_swarm_data=FargullHatcherSwarmData.from_json(json_data["fargull_hatcher_swarm_data"]),
        )
...
def _decode_flyer_swarm_data(data: typing.BinaryIO, game: Game, property_size: int) -> FlyerSwarmData:
    return FlyerSwarmData.from_stream(data, game, property_size)


def _decode_fargull_hatcher_swarm_data(
    data: typing.BinaryIO, game: Game, property_size: int
) -> FargullHatcherSwarmData:
    return FargullHatcherSwarmData.from_stream(data, game, property_size)
