...
class Counter(BaseObjectType):
    ...
    counter_conditions: CounterConditions = dataclasses.field(
        default_factory=CounterConditions,
        metadata={
            "reflection": FieldReflection[CounterConditions](
                CounterConditions,
                id=0x9C1D33DE,
                original_name="CounterConditions",
                from_json=CounterConditions.from_json,
                to_json=CounterConditions.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        counter_conditions = CounterConditions.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.counter_conditions.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_counter_conditions(data: typing.BinaryIO, game: Game, property_size: int) -> CounterConditions:
    return CounterConditions.from_stream(data, game, property_size)
