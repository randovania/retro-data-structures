...
class Rundas(BaseObjectType):
    ...
    unknown_struct55: UnknownStruct55 = dataclasses.field(
        default_factory=UnknownStruct55,
        metadata={
            "reflection": FieldReflection[UnknownStruct55](
                UnknownStruct55,
                id=0xA6790709,
                original_name="UnknownStruct55",
                from_json=UnknownStruct55.from_json,
                to_json=UnknownStruct55.to_json,
            ),
        },
    )
    rundas_data_0x9d90d49f: RundasData = dataclasses.field(
        default_factory=RundasData,
        metadata={
            "reflection": FieldReflection[RundasData](
                RundasData,
                id=0x9D90D49F,
                original_name="RundasData",
                from_json=RundasData.from_json,
                to_json=RundasData.to_json,
            ),
        },
    )
    ...
    rundas_data_0x877b2ce9: RundasData = dataclasses.field(
        default_factory=RundasData,
        metadata={
            "reflection": FieldReflection[RundasData](
                RundasData,
    ...
    rundas_data_0xce6183d8: RundasData = dataclasses.field(
        default_factory=RundasData,
        metadata={
            "reflection": FieldReflection[RundasData](
                RundasData,
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        unknown_struct55 = UnknownStruct55.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x9D90D49F
        rundas_data_0x9d90d49f = RundasData.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xB3774750
        patterned = PatternedAITypedef.from_stream(data, game, property_size, default_override={"leash_radius": 100.0})

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x877B2CE9
        rundas_data_0x877b2ce9 = RundasData.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x1464AE05
        patterned_ai_0x1464ae05 = PatternedAITypedef.from_stream(
            data, game, property_size, default_override={"leash_radius": 100.0}
        )

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xCE6183D8
        rundas_data_0xce6183d8 = RundasData.from_stream(data, game, property_size)

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x24D00673
        patterned_ai_0x24d00673 = PatternedAITypedef.from_stream(
            data, game, property_size, default_override={"leash_radius": 100.0}
        )
        ...
        return cls(
            editor_properties,
            unknown_struct55,
            rundas_data_0x9d90d49f,
            patterned,
            rundas_data_0x877b2ce9,
            patterned_ai_0x1464ae05,
            rundas_data_0xce6183d8,
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.unknown_struct55.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.rundas_data_0x9d90d49f.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.patterned.to_stream(data, game, default_override={"leash_radius": 100.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.rundas_data_0x877b2ce9.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.patterned_ai_0x1464ae05.to_stream(data, game, default_override={"leash_radius": 100.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.rundas_data_0xce6183d8.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.patterned_ai_0x24d00673.to_stream(data, game, default_override={"leash_radius": 100.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_unknown_struct55(data: typing.BinaryIO, game: Game, property_size: int) -> UnknownStruct55:
    return UnknownStruct55.from_stream(data, game, property_size)


def _decode_rundas_data_0x9d90d49f(data: typing.BinaryIO, game: Game, property_size: int) -> RundasData:
    return RundasData.from_stream(data, game, property_size)


def _decode_patterned(data: typing.BinaryIO, game: Game, property_size: int) -> PatternedAITypedef:
    return PatternedAITypedef.from_stream(data, game, property_size, default_override={"leash_radius": 100.0})


def _decode_rundas_data_0x877b2ce9(data: typing.BinaryIO, game: Game, property_size: int) -> RundasData:
    return RundasData.from_stream(data, game, property_size)
...
def _decode_rundas_data_0xce6183d8(data: typing.BinaryIO, game: Game, property_size: int) -> RundasData:
    return RundasData.from_stream(data, game, property_size)
