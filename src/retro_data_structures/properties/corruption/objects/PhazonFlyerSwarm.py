...
class PhazonFlyerSwarm(BaseObjectType):
    ...
    phazon_flyer_swarm_data: PhazonFlyerSwarmData = dataclasses.field(
        default_factory=PhazonFlyerSwarmData,
        metadata={
            "reflection": FieldReflection[PhazonFlyerSwarmData](
                PhazonFlyerSwarmData,
                id=0x100D1FD2,
                original_name="PhazonFlyerSwarmData",
                from_json=PhazonFlyerSwarmData.from_json,
                to_json=PhazonFlyerSwarmData.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        phazon_flyer_swarm_data = PhazonFlyerSwarmData.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.phazon_flyer_swarm_data.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_phazon_flyer_swarm_data(data: typing.BinaryIO, game: Game, property_size: int) -> PhazonFlyerSwarmData:
    return PhazonFlyerSwarmData.from_stream(data, game, property_size)
