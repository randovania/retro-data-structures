...
class SpacePirate(BaseObjectType):
    ...
    friendly_data: FriendlyData = dataclasses.field(
        default_factory=FriendlyData,
        metadata={
            "reflection": FieldReflection[FriendlyData](
                FriendlyData,
                id=0xE677AF2C,
                original_name="FriendlyData",
                from_json=FriendlyData.from_json,
                to_json=FriendlyData.to_json,
            ),
        },
    )
    space_pirate_info: SpacePirateData = dataclasses.field(
        default_factory=SpacePirateData,
        metadata={
            "reflection": FieldReflection[SpacePirateData](
                SpacePirateData,
                id=0xDDA1CACE,
                original_name="SpacePirateInfo",
                from_json=SpacePirateData.from_json,
                to_json=SpacePirateData.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        friendly_data = FriendlyData.from_stream(data, game, property_size, default_override={"is_grabbable": True})

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xDDA1CACE
        space_pirate_info = SpacePirateData.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.friendly_data.to_stream(data, game, default_override={"is_grabbable": True})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.space_pirate_info.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_friendly_data(data: typing.BinaryIO, game: Game, property_size: int) -> FriendlyData:
    return FriendlyData.from_stream(data, game, property_size, default_override={"is_grabbable": True})


def _decode_space_pirate_info(data: typing.BinaryIO, game: Game, property_size: int) -> SpacePirateData:
    return SpacePirateData.from_stream(data, game, property_size)
