...
class GhorLowerBody(BaseObjectType):
    ...
    ai_difficulty_settings: AiDifficultySettings = dataclasses.field(
        default_factory=AiDifficultySettings,
        metadata={
            "reflection": FieldReflection[AiDifficultySettings](
                AiDifficultySettings,
                id=0x42691490,
                original_name="AiDifficultySettings",
                from_json=AiDifficultySettings.from_json,
                to_json=AiDifficultySettings.to_json,
            ),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        ai_difficulty_settings = AiDifficultySettings.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.ai_difficulty_settings.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_ai_difficulty_settings(data: typing.BinaryIO, game: Game, property_size: int) -> AiDifficultySettings:
    return AiDifficultySettings.from_stream(data, game, property_size)
