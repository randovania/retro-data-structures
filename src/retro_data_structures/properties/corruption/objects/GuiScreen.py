...
class GuiScreen(BaseObjectType):
    ...
    which_screen: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0xD3B7E6D8, original_name="WhichScreen"),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        which_screen = structs.BIG_l.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xFD95ED2A
        string_table = structs.BIG_Q.unpack(data.read(8))[0]
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(structs.BIG_l.pack(self.which_screen))

        data.write(b"\xfd\x95\xed*")  # 0xfd95ed2a
        data.write(b"\x00\x08")  # size
        data.write(structs.BIG_Q.pack(self.string_table))
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    ...
    0xD3B7E6D8: ("which_screen", structs.decode_BIG_l),
    0xFD95ED2A: ("string_table", structs.decode_BIG_Q),
}
