...
class TweakBall_FireBall(BaseProperty):
    fire_ball_damage_delay: float = dataclasses.field(
        default=0.10000000149011612,
        metadata={
            "reflection": FieldReflection[float](float, id=0xBFDABD65, original_name="FireBallDamageDelay"),
        },
    )
    fire_ball_damage: TDamageInfo = dataclasses.field(
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        fire_ball_damage_delay = structs.BIG_f.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x2C6FC9EF
        fire_ball_damage = TDamageInfo.from_stream(
            data,
            game,
            property_size,
            default_override={
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(structs.BIG_f.pack(self.fire_ball_damage_delay))

        data.write(b",o\xc9\xef")  # 0x2c6fc9ef
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.fire_ball_damage.to_stream(
            data,
            game,
            default_override={
                "weapon_type": 5,
...
def _decode_fire_ball_damage(data: typing.BinaryIO, game: Game, property_size: int) -> TDamageInfo:
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    0xBFDABD65: ("fire_ball_damage_delay", structs.decode_BIG_f),
