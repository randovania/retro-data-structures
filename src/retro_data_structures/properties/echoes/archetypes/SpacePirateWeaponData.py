...
class SpacePirateWeaponData(BaseProperty):
    unknown_0x647670ac: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0x647670AC, original_name="Unknown"),
        },
    )
    grenade_launcher: AssetId = dataclasses.field(
        default=default_asset_id,
        metadata={
            "asset_types": ["CMDL"],
            "reflection": FieldReflection[AssetId](AssetId, id=0xA79BB82E, original_name="GrenadeLauncher"),
        },
    )
    unknown_0xa95a025b: int = dataclasses.field(
        default=3,
        metadata={
            "reflection": FieldReflection[int](int, id=0xA95A025B, original_name="Unknown"),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        unknown_0x647670ac = structs.BIG_l.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xA79BB82E
        grenade_launcher = structs.BIG_L.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xA95A025B
        unknown_0xa95a025b = structs.BIG_l.unpack(data.read(4))[0]
        ...
        grenade_damage = DamageInfo.from_stream(
            ...
            game,
            property_size,
            default_override={"di_weapon_type": 11, "di_damage": 40.0, "di_radius": 8.0, "di_knock_back_power": 10.0},
        )
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(structs.BIG_l.pack(self.unknown_0x647670ac))

        data.write(b"\xa7\x9b\xb8.")  # 0xa79bb82e
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_L.pack(self.grenade_launcher))

        data.write(b"\xa9Z\x02[")  # 0xa95a025b
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_l.pack(self.unknown_0xa95a025b))
    ...
    def _dependencies_for_grenade_launcher(self, asset_manager: AssetManager) -> Iterator[Dependency]:
        return asset_manager.get_dependencies_for_asset(self.grenade_launcher)
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    0x647670AC: ("unknown_0x647670ac", structs.decode_BIG_l),
    0xA79BB82E: ("grenade_launcher", structs.decode_BIG_L),
    0xA95A025B: ("unknown_0xa95a025b", structs.decode_BIG_l),
