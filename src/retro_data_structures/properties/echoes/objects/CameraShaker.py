...
class CameraShaker(BaseObjectType):
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        shaker_data = CameraShakerData.from_stream(data, game, property_size)
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.shaker_data.to_stream(data, game)
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
    ...
    def dependencies_for(self, asset_manager: AssetManager) -> Iterator[Dependency]:
        return self.shaker_data.dependencies_for(asset_manager)
...
def _decode_shaker_data(data: typing.BinaryIO, game: Game, property_size: int) -> CameraShakerData:
