...
class Spinner(BaseObjectType):
    ...
    loop_sound: int = dataclasses.field(
        default=0,
        metadata={
            "sound": True,
            "reflection": FieldReflection[int](int, id=0x8B66ECA2, original_name="LoopSound"),
        },
    )
    start_sound: int = dataclasses.field(
        default=0,
        metadata={
            "sound": True,
            "reflection": FieldReflection[int](int, id=0x52EDD16B, original_name="StartSound"),
        },
    )
    stop_sound: int = dataclasses.field(
        default=0,
        metadata={
            "sound": True,
            "reflection": FieldReflection[int](int, id=0xE88E7D41, original_name="StopSound"),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        loop_sound = structs.BIG_l.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x52EDD16B
        start_sound = structs.BIG_l.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xE88E7D41
        stop_sound = structs.BIG_l.unpack(data.read(4))[0]
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_l.pack(self.loop_sound))

        data.write(b"R\xed\xd1k")  # 0x52edd16b
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_l.pack(self.start_sound))

        data.write(b"\xe8\x8e}A")  # 0xe88e7d41
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_l.pack(self.stop_sound))
    ...
    def _dependencies_for_loop_sound(self, asset_manager: AssetManager) -> Iterator[Dependency]:
        return asset_manager.get_audio_group_dependency(self.loop_sound)

    def _dependencies_for_start_sound(self, asset_manager: AssetManager) -> Iterator[Dependency]:
        return asset_manager.get_audio_group_dependency(self.start_sound)

    def _dependencies_for_stop_sound(self, asset_manager: AssetManager) -> Iterator[Dependency]:
        return asset_manager.get_audio_group_dependency(self.stop_sound)
    ...
    def dependencies_for(self, asset_manager: AssetManager) -> Iterator[Dependency]:
        for method, field_name, field_type in [
            ...
            try:
                ...
                raise Exception(f"Error finding dependencies for Spinner.{field_name} ({field_type}): {e}")
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    ...
    0x8B66ECA2: ("loop_sound", structs.decode_BIG_l),
    0x52EDD16B: ("start_sound", structs.decode_BIG_l),
    0xE88E7D41: ("stop_sound", structs.decode_BIG_l),
}
