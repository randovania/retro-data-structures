...
class PlayerStateChange(BaseObjectType):
    ...
    item_to_change: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0x4A3491BD, original_name="ItemToChange"),
        },
    )
    ...
    command: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0x94BA5737, original_name="Command"),
        },
    )
    command_action: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0x363D98A4, original_name="CommandAction"),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        item_to_change = structs.BIG_l.unpack(data.read(4))[0]
        ...
        command = structs.BIG_l.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x363D98A4
        command_action = structs.BIG_l.unpack(data.read(4))[0]
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(structs.BIG_l.pack(self.item_to_change))
        ...
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_l.pack(self.command))

        data.write(b"6=\x98\xa4")  # 0x363d98a4
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_l.pack(self.command_action))
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    ...
    0x4A3491BD: ("item_to_change", structs.decode_BIG_l),
    0x28C71B54: ("capacity_increase", structs.decode_BIG_l),
    0x94AF1445: ("amount", structs.decode_BIG_l),
    0x94BA5737: ("command", structs.decode_BIG_l),
    0x363D98A4: ("command_action", structs.decode_BIG_l),
}
