...
class RsfAudio(BaseObjectType):
    ...
    unknown: str = dataclasses.field(
        default="",
        metadata={
            "reflection": FieldReflection[str](str, id=0xFE97E5B3, original_name="Unknown"),
        },
    )
    loop_start: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0x2E596612, original_name="LoopStart"),
        },
    )
    loop_end: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0xD8496DEA, original_name="LoopEnd"),
        },
    )
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        loop_start = structs.BIG_l.unpack(data.read(4))[0]

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0xD8496DEA
        loop_end = structs.BIG_l.unpack(data.read(4))[0]
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(structs.BIG_l.pack(self.loop_start))

        data.write(b"\xd8Im\xea")  # 0xd8496dea
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_l.pack(self.loop_end))
...
def _decode_unknown(data: typing.BinaryIO, game: Game, property_size: int) -> str:
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    ...
    0x2E596612: ("loop_start", structs.decode_BIG_l),
    0xD8496DEA: ("loop_end", structs.decode_BIG_l),
