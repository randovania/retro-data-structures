...
class VisorFlare(BaseObjectType):
    ...
    def _fast_decode(cls, data: typing.BinaryIO, game: Game, property_count: int) -> typing_extensions.Self | None:
        ...
        flare1 = FlareDef.from_stream(data, game, property_size, default_override={"scale": 1.0})

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x05A84EE6
        flare2 = FlareDef.from_stream(data, game, property_size, default_override={"position": 0.25, "scale": 1.0})

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x12D35AA5
        flare3 = FlareDef.from_stream(data, game, property_size, default_override={"position": 0.5, "scale": 1.0})

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x76B2376C
        flare4 = FlareDef.from_stream(data, game, property_size, default_override={"position": 0.75, "scale": 1.0})

        property_id, property_size = structs.BIG_LH.unpack(data.read(6))
        assert property_id == 0x61C9232F
        flare5 = FlareDef.from_stream(data, game, property_size, default_override={"position": 1.0, "scale": 1.0})
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.flare1.to_stream(data, game, default_override={"scale": 1.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.flare2.to_stream(data, game, default_override={"position": 0.25, "scale": 1.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.flare3.to_stream(data, game, default_override={"position": 0.5, "scale": 1.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.flare4.to_stream(data, game, default_override={"position": 0.75, "scale": 1.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b"\x00\x00")  # size placeholder
        self.flare5.to_stream(data, game, default_override={"position": 1.0, "scale": 1.0})
        after = data.tell()
        data.seek(before)
        data.write(structs.BIG_H.pack(after - before - 2))
        data.seek(after)
...
def _decode_flare1(data: typing.BinaryIO, game: Game, property_size: int) -> FlareDef:
    return FlareDef.from_stream(data, game, property_size, default_override={"scale": 1.0})


def _decode_flare2(data: typing.BinaryIO, game: Game, property_size: int) -> FlareDef:
    return FlareDef.from_stream(data, game, property_size, default_override={"position": 0.25, "scale": 1.0})


def _decode_flare3(data: typing.BinaryIO, game: Game, property_size: int) -> FlareDef:
    return FlareDef.from_stream(data, game, property_size, default_override={"position": 0.5, "scale": 1.0})


def _decode_flare4(data: typing.BinaryIO, game: Game, property_size: int) -> FlareDef:
    return FlareDef.from_stream(data, game, property_size, default_override={"position": 0.75, "scale": 1.0})


def _decode_flare5(data: typing.BinaryIO, game: Game, property_size: int) -> FlareDef:
    return FlareDef.from_stream(data, game, property_size, default_override={"position": 1.0, "scale": 1.0})
