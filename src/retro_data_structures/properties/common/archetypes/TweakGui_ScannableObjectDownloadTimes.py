...
class TweakGui_ScannableObjectDownloadTimes(BaseProperty):
    fast: float = dataclasses.field(
        default=1.0,
        metadata={
            "reflection": FieldReflection[float](float, id=0xEA8B1D10, original_name="Fast"),
        },
    )
    slow: float = dataclasses.field(
        default=3.0,
        metadata={
            "reflection": FieldReflection[float](float, id=0xB1338BEB, original_name="Slow"),
        },
    )
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        data.write(structs.BIG_f.pack(self.fast))

        data.write(b"\xb13\x8b\xeb")  # 0xb1338beb
        data.write(b"\x00\x04")  # size
        data.write(structs.BIG_f.pack(self.slow))
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, Game, int], typing.Any]]] = {
    0xEA8B1D10: ("fast", structs.decode_BIG_f),
    0xB1338BEB: ("slow", structs.decode_BIG_f),
}
