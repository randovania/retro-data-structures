...
class DistanceFog(BaseObjectType):
    ...
    mode: int = dataclasses.field(
        default=0,
        metadata={
            "reflection": FieldReflection[int](int, id=0x00000001, original_name="Mode"),
        },
    )
    ...
    range: Vector2f = dataclasses.field(
    ...
    color_delta: float = dataclasses.field(
        default=0.0,
        metadata={
            "reflection": FieldReflection[float](float, id=0x00000004, original_name="ColorDelta"),
        },
    )
    range_delta: Vector2f = dataclasses.field(
        default_factory=Vector2f,
        metadata={
            "reflection": FieldReflection[Vector2f](
                Vector2f,
                id=0x00000005,
                original_name="RangeDelta",
                from_json=Vector2f.from_json,
                to_json=Vector2f.to_json,
            ),
        },
    )
    explicit: bool = dataclasses.field(
        default=False,
        metadata={
            "reflection": FieldReflection[bool](bool, id=0x00000006, original_name="Explicit"),
        },
    )
    active: bool = dataclasses.field(
        default=False,
        metadata={
            "reflection": FieldReflection[bool](bool, id=0x00000007, original_name="Active"),
        },
    )
    ...
    def from_stream(
        ...
        range = Vector2f.from_stream(data, game, property_size)
        color_delta = structs.BIG_f.unpack(data.read(4))[0]
        range_delta = Vector2f.from_stream(data, game, property_size)
        explicit = structs.BIG_bool_.unpack(data.read(1))[0]
    ...
    def to_stream(self, data: typing.BinaryIO, game: Game, default_override: dict | None = None) -> None:
        ...
        self.range.to_stream(data, game)
        data.write(structs.BIG_f.pack(self.color_delta))
        self.range_delta.to_stream(data, game)
        data.write(structs.BIG_bool_.pack(self.explicit))
