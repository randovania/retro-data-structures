    ...
    def _parse(self, stream, context, path):
        ...
        if int(mrea_header.version) >= MREAVersion.Corruption.value:
            ...
            categories = [
                ...
                for label in _all_categories_echoes
...
class Mrea(BaseResource):
    ...
    def generated_objects_layer(self) -> ScriptLayer:
        ...
        gen_script_section = (
            "generated_script_objects_section" if int(self._raw.version) == MREAVersion.Echoes.value else "SGEN"
        )
        if self._generated_objects_layer is None:
            self._generated_objects_layer = ScriptLayer(self.get_section(gen_script_section)[0], None, self.target_game)
