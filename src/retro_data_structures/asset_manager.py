...
class PakExportStrategyCreate(PakExportStrategy):
    ...
    named_files_per_pak: dict[str, dict[str, AssetId]]

    def __init__(self, manager: AssetManager):
        super().__init__(manager)

        self.named_files_per_pak = {}
        for pak_name in manager.all_paks:
            mapping = manager.pak_group.get_named_resources_of_pak(pak_name)
            self.named_files_per_pak[pak_name] = dict(mapping)
    ...
    def export(self, output: FileWriter) -> None:
        ...
        for pak_name in modified_paks:
            ...
            id_order = []
            seen = set()
            for name, asset_id in self.named_files_per_pak[pak_name].items():
                seen.add(asset_id)
